...
检查 llm_result 字段中：
- success 是否为 True
- error 是否为 null（如果 success 为 True，error 应该为 null）
...
def check_llm_result(
    ...
    """
    检查 llm_result 字段的数据是否正确

    Args:
        llm_result: llm_result 数据字典
        record_id: 记录ID
        mode: 生成模式；为 cot_agent 时会额外检查 evidence_indices 与 evidence_indices_uncertain 至少一个非空
    ...
    if success is None:
        ...
        errors.append(f"success 不是布尔类型（实际类型: {type(success).__name__}）")
    elif success is not True:
        errors.append(f"success 为 False，期望为 True")

    # 检查 error 字段
    error = llm_result.get("error")
    if success is True:
        # 如果 success 为 True，error 应该为 null
        if error is not None:
            errors.append(f"success 为 True 但 error 不为 null（实际值: {error}）")
    else:
        # 如果 success 为 False，error 应该不为 null（但这不是必须的，只是记录）
        if error is None:
            errors.append("success 为 False 但 error 为 null（应该包含错误信息）")
    ...
    # cot_agent 模式：evidence_indices 与 evidence_indices_uncertain 至少一个非空（或标记了使用全量候选），且类型为列表
    if mode == "cot_agent":
        evidence_indices = llm_result.get("evidence_indices")
        evidence_indices_uncertain = llm_result.get("evidence_indices_uncertain")
        used_full_context = llm_result.get("used_full_context_as_candidates", False)
        if not isinstance(evidence_indices, list):
            errors.append(
                f"cot_agent 模式要求 evidence_indices 为列表（实际类型: {type(evidence_indices).__name__}）"
            )
        if not isinstance(evidence_indices_uncertain, list):
            errors.append(
                f"cot_agent 模式要求 evidence_indices_uncertain 为列表（实际类型: {type(evidence_indices_uncertain).__name__}）"
            )
        if isinstance(evidence_indices, list) and isinstance(evidence_indices_uncertain, list):
            has_evidence = len(evidence_indices) > 0 or len(evidence_indices_uncertain) > 0
            # 若标记了“使用全量 context 作为候选”，则视为合法（Agent 无候选时退化为全量+CoT，check 不判为错误）
            if not has_evidence and not used_full_context:
                errors.append("cot_agent 模式：evidence_indices 与 evidence_indices_uncertain 均为空（至少应有一个非空或 used_full_context_as_candidates=True）")
...
def check_single_file(
    ...
    Args:
        file_path: 结果文件路径
        mode: 生成模式（cot_agent 时会额外检查 evidence 相关字段）
    ...
    # 检查 llm_result 的各个字段（传入 mode 以便 cot_agent 时检查 evidence 字段）
    is_valid, errors = check_llm_result(llm_result, record_id, mode=mode)
...
def check_model_results(
    ...
    Args:
        ...
        mode: 生成模式（cot_agent 时会额外检查 evidence_indices / evidence_indices_uncertain）
    ...
    for file_path in json_files:
        is_valid, check_result = check_single_file(file_path, mode=mode)
        ...
        if is_valid:
            ...
            for error in errors:
                ...
                # 判断错误类型
                if "evidence_indices" in error or "evidence_indices_uncertain" in error:
                    stats["errors_by_type"]["evidence_indices/uncertain 均为空或异常"].append(record_id)
                elif "success" in error.lower():
                    stats["errors_by_type"]["success 字段错误"].append(record_id)
                elif "error" in error.lower() and "null" in error.lower():
                    stats["errors_by_type"]["error 字段错误"].append(record_id)
                elif "risk_of_bias" in error.lower():
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="检查生成的SSR证据提取结果文件"
    )
    parser.add_argument(
        "--results-dir",
        type=str,
        default=None,
        help="结果文件根目录路径（默认：项目根目录/data/ssr_evidence_results）"
    )
    parser.add_argument(
        "--model-name",
        type=str,
